"""
Experiment 3.3: Criticality in Multi-Agent Coordination (Reliability)
=====================================================================
Tests resilience to agent failure (silence/packet loss).

Variables:
- Reliability: {1.0, 0.9, 0.7, 0.5} (Prob of successful message)
- N: Number of agents (5)
- Topology: Fully Connected (to isolate reliability effect)
"""
...
def run_reliability_experiment(
    num_agents: int = 5,
    reliabilities: list = [1.0, 0.9, 0.7, 0.5],
    num_problems: int = 5,
    rounds: int = 3,
    output_dir: str = "results/criticality"
):
    print(f"EXPERIMENT 3.3: MULTI-AGENT RELIABILITY")
    print("=======================================")
    ...
    for rel in reliabilities:
        print(f"\nConfiguration: N={num_agents}, Reliability={rel}, Topology=Fully Connected")
        ...
        for i, problem in enumerate(problems):
            ...
            try:
                # Use Fully Connected to ensure failures stick out
                run_data = mas.run_debate(prompt, rounds=rounds, topology="fully_connected", reliability=rel)
        ...
        print(f"  Summary Reliability={rel}: Avg Consensus = {avg_consensus:.2%}")

    output_path = Path(output_dir)
    output_path.mkdir(parents=True, exist_ok=True)
    with open(output_path / f"exp3_3_reliability_N{num_agents}.json", 'w') as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--reliabilities", type=float, nargs="+", default=[1.0, 0.9, 0.7, 0.5])
